...
// =========================
// RECORTAR A 30 SEGUNDOS O LOOP SI <= 3s
// =========================
if (originalDuration <= 3) {
    console.log("Video muy corto, aplicando loop...");
    execSync(`ffmpeg -y -stream_loop -1 -i input_video.mp4 -t 30 -c:v libx264 -preset superfast -crf 28 -pix_fmt yuv420p video_cut.mp4`, { stdio: 'inherit' });
} else {
    console.log("Recortando a 30 segundos...");
...
// =========================
// SUBTÍTULOS / TEXTO SUPERIOR
// Texto limpio para ASS (sin emojis en el filter, se usan en drawtext solo si hay soporte)
// =========================
...
// =========================
// MEZCLAR AUDIO ORIGINAL + MÚSICA DE FONDO
// Audio original: 100% | Música de fondo: 20%
// =========================
// Primero extraer audio original del video recortado
execSync(`ffmpeg -y -i video_cut.mp4 -vn -c:a aac -b:a 128k -ar 48000 original_audio.aac`, { stdio: 'inherit' });

// Loop música de fondo al largo del video y bajar volumen
